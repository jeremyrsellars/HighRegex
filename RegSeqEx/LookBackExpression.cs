...
{
   /// <summary>
   /// A zero-width assertion that the expression is preceeded by the specified expression.
   /// </summary>
   /// <typeparam name="T">The type of item matched by a regular expression</typeparam>
   public class LookBackExpression<T> : IExpression<T>, ILookBackMatchProvider
   {
      /// <summary>
      /// Creates an expression that has no width, but will match if contained expression matches immediately before the current position.
      /// </summary>
      /// <param name="expression">The expression to compare with the input items at the current position.</param>
      public LookBackExpression (IExpression<T> expression)
      ...
      protected internal LookBackExpression (IExpression<T> expression, bool negate)
      {
         if (expression == null)
      ...
      /// the contained expression does not match the input immediately preceeding the current index.
      /// </summary>
      ...
      {
         // Technically, the maximum length of a lookback match is the current index.
         foreach (int possibleMatchLength in m_lookBack.GetPossibleMatchLengths (index))
         {
            foreach (var match in m_expression.GetMatches (input, index - possibleMatchLength))
            {
               bool isMatch = match.Length == possibleMatchLength;
               if (isMatch)
                  return MatchLength.EmptyMatch(Negate ^ isMatch);
            }
         }
         
         return MatchLength.EmptyMatch(Negate);
      }
